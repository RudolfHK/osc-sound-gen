...
export function TransportBar() {
  ...
  const handlePlay = useCallback(async () => {
    if (seq.isPlaying) {
      ...
      try {
        ...
        dispatch({ type: 'SEQ_SET_PLAYING', playing: false });
        setSeqError('Audio failed to start. Try clicking Play on an oscillator tab first.');
      }
    }
  ...
  return (
    <div className="flex flex-col border-b border-neutral-800 bg-neutral-900/50 shrink-0">
      {/* Error banner */}
      {seqError && (
        <div className="px-3 py-1 text-xs text-red-400 font-mono bg-red-950/30 border-b border-red-900/50">
          ⚠ {seqError}
        </div>
      )}
      ...
      <div className="flex items-center gap-3 px-3 py-2 flex-wrap">
        ...
        {/* Default note length */}
        <div className="flex items-center gap-1" title="Duration of newly drawn notes">
          <span className="text-xs text-neutral-600 tracking-widest">NOTE</span>
          <select
            value={seq.defaultNoteLength}
            className="bg-neutral-950 border border-neutral-700 text-xs text-neutral-200 px-1 py-0.5 focus:outline-none"
            onChange={(e) => dispatch({ type: 'SEQ_SET_DEFAULT_NOTE_LEN', len: e.target.value as SnapValue })}
          >
            {SNAP_OPTIONS.map((s) => <option key={s} value={s}>{s}</option>)}
          </select>
        </div>
        ...
        <button
          ...
          style={seq.showVelocityLane ? { borderColor: accent, color: accent, backgroundColor: accent + '18' } : {}}
          className={`px-2 py-1 text-xs border tracking-widest transition-colors ${
            !seq.showVelocityLane ? 'border-neutral-700 text-neutral-600 hover:border-neutral-500 hover:text-neutral-300' : ''
          }`}
        ...
        <button
          ...
          disabled={seq.selectedNoteIds.length === 0}
          ...
          title="Quantize selected notes to snap grid (Q)"
        >
          Q
        </button>
